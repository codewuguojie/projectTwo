<template>
    <div class="wraper">
        ...
        <div class="pointer" :style="{width:lotterywidth[1]}" @click="clickLottery">
            <img :src="pointerbg" style="width: 70%; height: 70%;">
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        watch:{
            lotteryPrizeno(curVal,oldVal){
                if(curVal) {
                    console.log('这个是监听')
                    console.log(oldVal)
                    console.log(curVal)
                    oldVal = curVal
                    this.lotteryPrizeno = curVal;
                }
            }
        },
        computed: {
            difference () {
                console.log('这个是difference')
                console.log(this.prizeNo)
                return (this.prizeNo / this.prizeNum) * 360 - 360 / this.prizeNum / 2
            },
            prizeNo () {
                console.log('这个是进入prizeNo时的num')
                console.log(this.lotteryPrizeno)
                ...
                function cicle () {
                    if (self.angle < oneAngel) {
                        ...
                        if (speed <= 0) {
                            ...
                            self.flag = true
                            console.log('这个是result')
                            console.log(self.prizeNo)
                            console.log(self.prizeNum)
